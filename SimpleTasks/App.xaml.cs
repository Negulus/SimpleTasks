...
namespace SimpleTasks
{
    public partial class App : Application
    {
        ...
        {
            AppData.SetLoad();
            AppData.DB = new TasksDC(TasksDC.DBConnectionString);
            if (AppData.DB.DatabaseExists() == false)
                AppData.DB.CreateDatabase();
            AppData.TaskStart();
            ...
            {
                bool update = false;
                foreach (var item in TasksListDB.OrderBy(i => i.Item_TimeEnd))
                {
                    if (item.Item_End && DateTime.FromBinary(item.Item_TimeEnd).CompareTo(DateTime.Now) < 0)
                    {
                        (AppData.DB.Items.FirstOrDefault(i => i.Item_ID == item.Item_ID)).Item_Ended = true;
                        update = true;
                    }
                    else
                    {
